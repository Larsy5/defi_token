...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        ...
        return msg.data;
        // msg.data is used to handle array, bytes, string
    }
}
...
contract POWERPERPETUALMOTIONMACHINE is Context, IBEP20, Ownable {// contract name
    ...
    address payable public _marketingAddress = payable(0xF35284dBf6a20250561F8D828a050B18a703aBE9); // Marketing Address
    address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Power Perpetual Motion Machine"; // token name
    string private _symbol = "PPMM"; // token symbol
    ...
    uint256 public tradingEnabledTimestamp = 1628604000;
    ...
    uint256 public _marketingBNBFee = 40; // marketing BNB fee
    ...
    uint256 public _maxTxAmount = _tTotal.mul(5).div(1000); // max allowed tokens tranfer per transaction
    ...
    uint256 public _maxTokensPerAddress = _tTotal.mul(5).div(1000); // Max number of tokens that an address can hold 5% of total supply
    ...
    constructor (uint _botSeconds) {
        botSeconds = _botSeconds;

        _tOwned[owner()] = _tTotal;
        // assigning the max token to owner's address
    ...
    function setBotSeconds(uint second) public onlyOwner{
        botSeconds = second;
    }
    ...
    */
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[from] || _isExcludedFromFee[to] || getTradingIsEnabled(),
        ...
        if (
            ...
            block.timestamp <= tradingEnabledTimestamp + botSeconds) {
            addBot(to);
        }
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 swapTokens = tokenBalance.mul(swapPercent).div(_deductableFee);
    ...
    function _randomBuyBackTime() private returns (uint256){
        uint randomBuyBackTime = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 1 days;
        randomBuyBackTime = randomBuyBackTime + durationBetweenEachBuyback;
        nonce++;
        return randomBuyBackTime;
    }
